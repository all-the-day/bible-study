...
- 「我的笔记」的划线汇总**跟随当前章篇目对应**：只列本章经文标注 + 当前章对应生命读经篇目（自动匹配或手动 `lrMap`）上的标注；按来源分 tab（全部/经文/生命读经），点击跳回原文。**划线汇总（`renderHighlights`，研读列与阅读器右栏共用）展示全部标记不过滤**，但区分笔记：头部计数 `划线汇总 N · 笔记 M`（带 note 计数，切 tab 同步刷新），组内**带笔记条目置顶**。**全局聚合已移出研读列**：研读列只保留本章聚合，全量标注/大段笔记的浏览管理在首页「我的笔记」块直进的**笔记管理模块**（见「首页 + 合集块」节）
