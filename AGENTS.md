...
- **书报模块（books，倪柝声文集，后续系列同构扩展）**：entry 直接进上次位置（`LS_BOOK_LAST`{volume,book,chapter}，无则第1辑第1本第1章）；左栏=辑条（`.bk-vol-strip`）+ 书列表（`.bk-nav-books`）；主区=`#bookMain` 当前章正文（`.bk-para`/`.bk-head lr-h{n}` 按行 data-base 渲染：**标题行 `detectLrHeading` 识别独立成 `.bk-head`（左对齐+加粗+字号递进，无缩进），正文段落纯文本样式无卡片（2026-08 用户定稿：去掉卡片）**；标注 type 'book' 坐标系=chapter.content）；右栏=`#bookSide`「章列表|笔记」（章列表切章，笔记=章级 textarea `state.bookNotes` + 标注汇总）；crumb=`倪柝声文集 · 书名 · 第N章`；数据 `data/books/`：`ni.json` 元数据（46KB，含章标题）+ `ni-{辑}.json` 按辑懒加载（3 辑 62 本 1325 章，共约 24MB）；切辑/切书/切章 `selectBookVolume/selectBookItem/selectBookChapter`（注意：**书报函数已加 Item 前缀避免与读经 selectBook/renderBookList 同名覆盖**）；统一入口 `openBookChapter(volume, book, chapter)`
