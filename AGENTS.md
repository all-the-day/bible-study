...
| `vendor/vconsole.min.js` | vConsole 调试台（本地 vendor 不走 CDN，APK 离线可用）。**调试模式默认关闭**：设置弹窗底部版本号 3 秒内连点 5 次切换（`toggleDebugMode`，LS_VCONSOLE 持久化、重启自动恢复）；Network 面板可看同步请求状态码/耗时，排障用（Storage 面板含同步令牌等敏感信息，用完关闭） |
