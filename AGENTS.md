...
- **状态**：`state.screen`（'home'|'work'，唯一视图正交开关）、`state.activeModule`（当前阅读器模块：'bible'|'lifereading'）、`state.notesScope`（'chapter'|'global'）、`state.lrVolumes`（生命读经卷懒加载缓存，`selectChapter` 懒加载与阅读器共用）、`state.lrBookIndex/lrArticleId/lrSideTab`（生命读经阅读器位置与右栏 tab）、`state.histStack/histPos/source/scrollCache`（导航：历史栈/来源链/滚动缓存，见「导航：历史栈 + 来源链」）
- **导航：历史栈 + 来源链（P0，2026-08）**：顶栏新增 `#histBackBtn/#histFwdBtn`（‹ › 前进后退）+ 主区顶部 `#sourceChain` 来源链条（sticky 黄条：`从「XX」进入` + 返回来源按钮）。**来源链只记上一跳**：跨模块/关联跳转（生命读经右栏「关联」卡 `followChapterFromLr`、全局笔记 `navigateToAnnotation` 跨模块分支）经 `rememberNav()` 设置来源+入栈；`goBackSource()` 返回来源位置后清除；**用户手动翻章/翻篇/切卷/回首页（`showHome`）清除**（select 函数统一 `opts.keepSource` 守卫：程序化跳转 `jumpToLoc`/标注跳转传 keepSource 保留，用户直接交互默认清除）。**历史栈只记跨模块/关联跳转**（首页块/搜索入口 `enterModuleFromHome` 入栈；同位置去重 `sameLoc`、上限 100）；`jumpToLoc(e)` 统一按快照跳转（跨模块 enterModule / 模块内 select），并恢复目标滚动（`scrollCache` 按位置键存 scrollTop）。快照 `snapshotLoc()` 含 `{module, label, ...位置字段}`，label 为跳转前生成的位置描述
...
- **生命读经模块（lifereading，方案 A 三列）**：entry 直接进上次篇目（`LS_LR_LAST`，无则第一卷第一篇）；左栏=卷条（66 卷横向 `.lr-vol-strip`）+ 当前卷篇目列表（`.lr-nav-articles`），☰ 折叠与读经一致；主区=篇目正文（`renderLrArticle(art, bookIndex)` 渲染到 `#lrMain`，标注坐标系不变）；右栏=`#lrSide`「纲目|关联|笔记」tab（纲目在前：`extractLrHeadings` + 滚动高亮 `bindLrOutlineSpy` 句柄化监听 `#textCol`；**关联=篇→章反向**：解析本篇 `verses` 去重列出涉及章节（含节范围），点击经 `followChapterFromLr` 回典籍对应章并记来源链；笔记：篇级 textarea `state.lrNotes` + 本篇标注汇总）；crumb=`卷名 · 第{id}篇 {标题}` 点击弹篇目列表；actions 只留 ⚙️（`body-mod-lifereading` CSS 隐藏其余）；切篇/切卷 `selectLrArticle/selectLrVolume` 持久化 LS_LR_LAST + 正文滚顶；统一入口 `openLrArticle(bookIndex, art)`（模块内切篇/模块外进模块）
